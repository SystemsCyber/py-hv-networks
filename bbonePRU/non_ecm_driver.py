...
""" non_ecm_driver.py: loads the appropriate PRU binary into the appropriate PRU core, then
...
class PRUReadThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stopped.is_set():
            ...
            pypruss.clear_event(1, pypruss.PRU1_ARM_INTERRUPT)                                              # Clear the event
            self.calls += 1
...
#                print(",".join(map(lambda x: "{:02x}".format(x), message)))
                self.socket.sendto(''.join(map(chr,message)), ('localhost', 6972))
...
ddr_addr = 0x4a302000 #0x4a300000
ddr_size = pypruss.ddr_size()
